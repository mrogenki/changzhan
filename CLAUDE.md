...
- **每人費用 `fee`**（0 = 免費）：公開頁顯示金額，帶人時會即時算「共 N 位 · 應付 X 元」；轉成收款時直接當預設金額。
- **綁定活動時會把活動資訊帶過來**：選了活動就自動填入該活動的 `price`（費用）與日期時間（截止時間），主題空白時也帶入活動標題。**`fee` 與 `activities.price` 是兩個獨立欄位**，刻意不鎖死（接龍可能只收餐費），但兩者不一致時表單會跳黃色提醒，並明說「報名頁顯示的是接龍這裡的金額」。公開頁的日期／時間／地點一律取自綁定的活動，說明欄不需要重打。
